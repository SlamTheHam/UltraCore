			...
			if (args[0].equalsIgnoreCase("pos")) {
				if (args.length == 1) {
					...
					if (!sender.hasPermission("core.tp")) {
						...
						Location loc = new Location(player.getLocation().getWorld(), Integer.parseInt(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3]));
						if (loc == null) {
							player.sendMessage(ChatColor.RED + "Location invalid or not typed correctly do /teleport help or contact an admin for help");
							return true;
						}
						player.teleport(loc);
						player.sendMessage(ChatColor.YELLOW + "You have been teleported to: " + ChatColor.GREEN + "X: " + args[1] + ", Y: " + args[2] + ", Z: " + args[3]);
